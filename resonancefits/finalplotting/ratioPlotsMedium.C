void ratioPlotsMedium(){
    TH1D* totHist1;
    TH1D* totHist2;
    TH1D* totBarHist1;
    TH1D* totBarHist2;
    TFile* kstarbarReconMed  = new TFile("20170616_Kstar0bar_masswidth_recon_pf100_smallrange_scaled.root");
    TFile* kstarbarDecayMed = new TFile("20170616_Kstar0bar_masswidth_pf160_smallrange_scaled.root");
    ...
    for(int i = 0; i < 20; i++){
        if(i <5){  
            TH1D* hist1 = kstarbarReconMed->Get(Form("ptbin0%iparticle5", (i*2)+1));
            TH1D* hist2 = kstarbarDecayMed->Get(Form("ptbin0%iparticle5", (i*2)+1));
            TH1D* barhist1 = kstarbarRecon->Get(Form("ptbin0%iparticle5", (i*2)+1));
            TH1D* barhist2 = kstarbarDecay->Get(Form("ptbin0%iparticle5", (i*2)+1));
        }else{
            TH1D* hist1 = kstarbarReconMed->Get(Form("ptbin%iparticle5", (i*2)+1));
            TH1D* hist2 = kstarbarDecayMed->Get(Form("ptbin%iparticle5", (i*2)+1));
        ...
        if(i==0){
            hist1->GetFunction("fitPTbin100particle5")->Delete();
            hist2->GetFunction("fitPTbin100particle5")->Delete();
    ...
    legend->AddEntry(ratio, "#bar{K}*^{0} Reconstructed/Decay w in-med.", "lp");
    legend->AddEntry(barratio, "#bar{K}*^{0} Reconstructed/Decay w/o in-med.", "lp");
    ...
    TLegend* legend2 = new TLegend(0.562814, 0.668412, 0.860553, 0.926702);
    legend2->AddEntry(totHist2, "#bar{K}*^{0} decay w in-med.", "l");
    legend2->AddEntry(totHist1, "#bar{K}*^{0} recon. w in-med.", "l");
    legend2->AddEntry(totBarHist2, "#bar{K}*^{0} decay w/o in-med.", "l");
    legend2->AddEntry(totBarHist1, "#bar{K}*^{0} recon. w/o in-med.", "l");
    legend2->SetMargin(0.15);
    legend2->SetTextSizePixels(20);
    legend2->SetTextAlign(22);
    legend2->Draw("SAME");

    TLegend* legend3 = new TLegend(0.17714, 0.790436, 0.461055, 0.945759);
    legend3->AddEntry(gratio, "#bar{K}*^{0} ratio w in-med.", "lp");
    legend3->AddEntry(gbarratio, "#bar{K}*^{0} ratio w/o in-med.", "lp");
    legend3->SetMargin(0.2);
    legend3->SetTextSizePixels(22);
