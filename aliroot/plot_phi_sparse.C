...
void plot_phi_sparse(string inputName){
    gROOT->Reset();
    ...
    fHK0Dphi[7]->SetTitle("p_{T}^{h} > 7,   2 < p_{T}^{K} < 4 GeV/c");
    fHK0Dphi[8] = HK0Dphi->ProjectionZ("ptH7_4ptK0", 140, 1000, 81, 1000);
...
*/    
    ...
    for(int i=-1; i<3; i++){
        ...
        if(i+2 == 1){
            ...
            heading.SetTextAlign(22);
            heading.SetTextSize(0.1);
            heading.DrawLatex(0.5, 0.5, "#splitline{#Delta#phi correlation between}{#splitline{hadrons and K^{0} mesons,}{p_{T}^{#phi} inclusive}}");            
        }else{
...
/*
    ...
    for(int i=5; i<9; i++){
        ...
        if(i-4 == 1){
            ...
            heading.SetTextAlign(22);
            heading.SetTextSize(0.1);
            heading.DrawLatex(0.5, 0.5, "#splitline{#Delta#phi correlation between}{#splitline{high p_{T} hadrons (> 7 GeV/c)}{and K^{0} mesons}}");
         }else{
            fHK0Dphi[i]->GetXaxis()->SetTitle("#Delta#phi");
            fHK0Dphi[i]->SetLineWidth(0);
            fHK0Dphi[i]->SetLineColor(0);
