...
void histo_gen_test(string inputDir, string inputFile, string outputDir, string outputFile){
	TStopwatch timer_i;
	timer_i.Start();
 ...
 //   Int_t numEvents = rl->GetNumberOfEvents();
    ...
    timer_i.Stop();
    printf("Time for initial set-up/opening: ");
    timer_i.Print("u");

    //Loop over all events
    TStopwatch timer_event;
    TStopwatch timer_event_load;
    for(Int_t nev=0; nev<numEvents; nev++){

        timer_event_load.Start();
...
`        rl->GetEvent(nev);
        ...
        timer_event_load.Stop();
        printf("Time to load event: ");
        timer_event_load.Print("u");
        ...
        timer_event.Stop();
        printf("Time for event analysis: ");
        timer_event.Print("u");
    }
...
//    printf("Total hadrons counted in eta range: %d\n", count);
    ...
    timer_o.Stop();
    printf("Time for file writing/closing: ");
    timer_o.Print("u");
}
