...
// create and customize the alien handler
  AliAnalysisAlien *alienHandler = new AliAnalysisAlien();
    ...
    alienHandler->SetAdditionalLibs("AliAnalysisTaskPhiEff.cxx AliAnalysisTaskPhiEff.h AddTaskEff.C libpythia6.so libEGPythia6.so libAliPythia6.so libPWGHFhfe.so libCDB.so libSTEER.so libCORRFW.so libPWGflowBase.so libPWGflowTasks.so libGui.so libProof.so libMinuit.so libXMLParser.so libRAWDatabase.so libRAWDatarec.so libCDB.so libSTEERBase.so libSTEER.so libTPCbase.so libTOFbase.so libTOFrec.so libTRDbase.so libVZERObase.so libVZEROrec.so libT0base.so libT0rec.so libPWGTools.so libPWGCFCorrelationsBase.so");

   
  alienHandler->SetAnalysisSource("AliAnalysisTaskPhiEff.cxx");
